...
The attached Python script is a full example implementation for calling the BloodHound APIs using the API key/ID pair: <Icon icon="paperclip" iconType="solid" size={24} color="#5465ff" /> <a href="/assets/apiclient.zip" download>apiclient.zip 4KB</a>

The following is a code snippet from the attached script that illustrates the process specific to calling the API:
