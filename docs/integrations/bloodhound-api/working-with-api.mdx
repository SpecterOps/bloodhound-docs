...
* An API key/ID pair is generated within the Administration interface. In BloodHound Enterprise v9.0.0 and later, API keys can expire based on your tenant policy and are primarily used for API integrations.
...
<Note>
In BloodHound Enterprise v9.0.0 and later, both personal and non-personal API keys follow your tenant API key expiration policy.
</Note>
...
### Understand API key expiration (BloodHound Enterprise)

BloodHound Enterprise supports API key expiration to help you align with internal security and compliance requirements.

Expiration applies equally to all three API key types:
...
| --- | --- |
| Default validity period | New keys expire after 90 days by default. |
| Custom validity period | Administrators can set key validity from 1 to 365 days. |
| Feature turned on | Existing keys receive expiration based on the configured value. New keys follow the same policy. |
| Validity period changed | Each key keeps the earlier of its current expiration and now + configured days. |
| Feature turned off | Expiration enforcement stops, expiration dates are cleared, and keys no longer expire. |

<Warning>
BloodHound does not automatically rotate API keys. You must manually create replacement keys and update all dependent integrations and collector client configurations before existing keys expire.
</Warning>

To configure this setting, see [Configure API key expiration](/manage-bloodhound/auth/api-key-expiration).
