...
* A JWT is generated through the login process using your email, password, and 2FA token (or SAML-based authentication flow). These JWT tokens are active for 8 hours and are primarily for end-user access to the web-based application.
...
* Non-personal API key/ID pairs for integrations like [Splunk](/integrations/splunk/install)
