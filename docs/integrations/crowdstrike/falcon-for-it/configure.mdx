...
This guide shows you how to install the Foundry application, configure the BloodHound Enterprise connection, set up the required OS queries, and confirm that telemetry reaches Falcon Next-Gen SIEM.
...
This section shows you how to save and schedule the required OS queries in the CrowdStrike Falcon console so your target Windows hosts continuously send data to Next-Gen SIEM.
...
<Steps>
  ...
  <Step title="Save each required OS query">
    1. Paste one required OS query provided for the integration into the query input.

    1. Open the three-dot menu and select **Save As**.

    1. Enter a name for the query and save it.

    <Note>The vendor guide states that you must save the required OS queries one at a time.</Note>
  </Step>

  <Step title="Schedule the query">
    1. Open the menu next to **Run** and select **Schedule**.

    1. Set **Type** to **Recurring**.
  ...
  <Step title="Repeat for the remaining OS queries">
    Repeat the save and schedule process for each remaining query required by the integration.
  </Step>
...
Confirm that CrowdStrike routes the OS query results into Falcon Next-Gen SIEM before you run the BloodHound Enterprise workflow.
...
<Steps>
  ...
  <Step title="Filter for OS query telemetry">
  ...
  <Step title="Confirm that events are present">
    ...
    <Frame>
      <img
        ...
        alt="A screenshot of the filter interface for OS query telemetry"
        noZoom
      />
    </Frame>
  </Step>
