...
>
  The CrowdStrike Falcon for IT integration collects endpoint data from Falcon-managed hosts and ingests that data into BloodHound Enterprise.

  | | |
  | --- | --- |
  | **Supported actions** | <ul><li>Collect sessions, local groups, user rights assignment, and registry data from selected CrowdStrike host groups.</li><li>Schedule recurring FFIT queries and route the results into CrowdStrike Next-Gen SIEM.</li><li>Transform collected events and ingest them into BloodHound Enterprise automatically.</li></ul> |
  | **Common use cases** | <ul><li>Automate endpoint collection for BloodHound Enterprise by using Falcon-managed agents.</li><li>Scope collection to selected host groups instead of the full Falcon fleet.</li><li>Improve collection coverage by retrying incomplete host reporting.</li></ul> |
