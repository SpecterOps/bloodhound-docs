...
<Card
  title="CrowdStrike Falcon for IT"
  href="/integrations/crowdstrike/falcon-for-it/configure"
>
  The BloodHound Enterprise CrowdStrike Falcon Foundry Application is a native serverless integration that automatically collects sessions, local groups, user rights assignment (LSA), and registry key data from CrowdStrike Falcon agents on endpoint hosts and ingests that data into BloodHound Enterprise.
  
  This integration enables security teams to enrich Active Directory and Azure attack path analysis with endpoint telemetry collected from their existing CrowdStrike Falcon deployment.

  | | |
  | --- | --- |
  | **Supported actions** | <ul><li>Install directly from CrowdStrike Marketplace by using a guided setup wizard.</li><li>Securely configure BHE API credentials through the Foundry Secret Store.</li><li>Automatically provision and schedule Falcon for IT (FFIT) queries for sessions, local groups, LSA, and registry data.</li><li>Scope collection to specific Falcon host groups.</li><li>Route collected data into Falcon Next-Gen SIEM (LogScale) in real time.</li><li>Use Foundry Functions to fetch, transform, and ingest data into BHE.</li><li>Track host reporting state through a Daily Status Table in Foundry Collections.</li><li>Retry and reconcile incomplete collection runs to maintain host coverage.</li></ul> |
  | **Common use cases** | <ul><li>Enrich BloodHound Enterprise attack path analysis with endpoint session and privilege data.</li><li>Maintain continuous, hands-off data collection across the endpoint fleet.</li><li>Track and reconcile which hosts reported data during a 24-hour cycle.</li><li>Extend BloodHound coverage to hosts as soon as they come online.</li><li>Reduce manual configuration effort through pre-built FFIT query workflows.</li><li>Keep BHE data fresh without relying on manual pipelines or external storage.</li></ul> |
  | **Integration instructions** | <a href="/integrations/crowdstrike/falcon-for-it/configure">Configure the CrowdStrike Falcon for IT integration</a> |
</Card>
