---
title: Understand the Google SecOps design
description: Technical reference for the BloodHound Enterprise Google SecOps integration, including architecture, authentication, connector flow, and API usage.
...
<Note>
  The functional specification defines de-duplication by tracking the latest processed finding timestamp for each environment and finding type combination.
</Note>
...
| Action | Purpose | Primary API usage |
| --- | --- | --- |
| **Ping** | Verifies that Google SecOps can reach the BloodHound Enterprise server with the configured credentials. | Signed request to the BloodHound Enterprise API |
| **Get Object Id** | Searches for a node by name and returns matching object IDs. | `GET /api/v2/search` |
| **Does Path Exist** | Checks whether a shortest path exists between two specified nodes. | `GET /api/v2/graphs/shortest-path` |
| **Fetch Assets** | Retrieves detailed asset information for an object ID. | `GET /api/v2/azure/{entity_type}` for Azure entity lookups called out in the functional specification |

## BloodHound API dependencies
...
| `GET /api/v2/assets/findings/{finding_type}/title.md` | Returns the human-readable title for a finding type |
