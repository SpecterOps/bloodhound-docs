...
- BloodHound Enterprise [non-personal API key/ID pair](/integrations/bloodhound-api/working-with-api#create-a-personal-api-key-and-id-pair)
  ...
  <Note>At minimum, the user account associated with the API key/ID pair must have the **Read-only** role. The **Auditor** or **Administrator** role is required to collect BloodHound Enterprise audit logs.</Note>
