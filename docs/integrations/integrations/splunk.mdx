...
<Steps>
  ...
  <Step title="Configure Splunk app API credentials">
    ...
    <Note>
      - We recommend a [non-personal API key/ID pair](/integrations/bloodhound-api/working-with-api#create-a-personal-api-key-and-id-pair) for API integrations.
      - At minimum, the user account associated with the API key/ID pair must have the **Read-only** role. The **Administrator** role is required to collect BloodHound Enterprise audit logs.
    </Note>
  ...
  <Step title="Configure Splunk data inputs">
    ...
    1. Complete the configuration fields for the selected input type.
        ...
        | **Bloodhound Account**         | Bloodhound Enterprise account ([configured in Splunk](#configure-bloodhound-splunk-app)) that will be used to fetch the data      |
...
- Configure a Splunk search macro
- Configure a Splunk proxy
