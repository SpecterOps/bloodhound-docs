...
AdminSDHolder is both an object and an associated background task which runs on the domain controller holding the PDCe FSMO role. Any modifications made to the security descriptor of the AdminSDHolder container will be tattooed on the target node by the ProtectAdminGroups background task (not SDProp) every hour by default.

## Abuse Info

Any modifications to the AdminSDHolder node's security descriptor via inbound [Owns](/resources/edges/Owns), [WriteOwner](/resources/edges/WriteOwner), or
[WriteDACL](/resources/edges/WriteDACL) edges will propagate to all nodes with an inbound ProtectAdminGroups edge from the originating
...
- [SpecterOps: AdminSDHolder: Misconceptions and Myths](https://specterops.io/)
