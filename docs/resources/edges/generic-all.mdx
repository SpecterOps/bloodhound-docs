...
If you want to be more targeted with your approach, it is possible to specify precisely what right you want to apply to precisely which kinds of descendant objects. You could, for example, grant a user "ForceChangePassword" privilege against all user objects, or grant a security group the ability to read every GMSA password under a certain OU. Below is an example taken from PowerView's help text on how to grant the "ITADMIN" user the ability to read the LAPS password from all computer objects in the "Workstations" OU:
```json
$Guids = Get-DomainGUIDMap
...
See the the sections "Generic Descendant Object Takeover" and "Targeted Descendant Object Takeover" under With GenericAll Over an OU.
