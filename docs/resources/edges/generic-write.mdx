...
GenericWrite also grants write access to the `altSecurityIdentities` attribute, which may enable an ADCS ESC14 Scenario A attack. See the [WriteAltSecurityIdentities](/resources/edges/write-alt-security-identities) edge documentation for details.
