...
<Update label="BloodHound" description="Enhancement" tags={["Cypher"]}>
  ## Expanded path introspection functions
  {/*BED-8294, BED-8261*/}

  Query path contents more flexibly with support for `nodes(path)` result mapping and `relationships(path)`, `startNode()`, and `endNode()` traversal expressions.

  These updates improve compatibility with common Cypher query patterns that depend on iterating or returning ordered path components.
</Update>
