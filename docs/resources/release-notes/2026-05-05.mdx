...
<Update label="BloodHound" description="Enhancement" tags={["Data Ingestion"]}>
  {/*BED-8131*/}
  {/*BED-8098*/}
  {/*BED-8097*/}
  ## Ingest and Post-Processing

  Process large datasets with faster traversal paths and less <Tooltip tip="A phase of analysis in which BloodHound models structural aspects that are inherent to a given system but are not directly collected as data." cta="Learn more" href="/analyze-data/findings/analysis#post-processing">post-processing</Tooltip> churn.

  This release optimizes graph index coverage for common traversal paths and introduces "delta-change apply" behavior for some post-processed edges.
  
  As a result, BloodHound now updates the following edges only when they have changed, instead of deleting and recreating them in full during each post-processing run:
...
<Update label="BloodHound" description="Enhancement" tags={["OpenGraph"]}>
  ...
  Manage OpenGraph extensions with more granular API-based control.

  Extension management API endpoints now support finer-grained permissions for extension administration and related schema data access.

  {/* TODO: Link to updated API reference when available. */}
</Update>
