...
- **New modeling for AdminSDHolder** - Added a new [`ProtectAdminGroups`](/edges/protect-admin-groups) non-traversable edge between the `AdminSDHolder` Active Directory container in each domain and all user, computer, and group nodes that it protects. The `ProtectAdminGroups` edge tracks the relationship between the `AdminSDHolder` security descriptor and protected nodes. This enhancement helps identify privilege escalation paths involving Tier Zero groups and improves risk analysis for environments (requires SharpHound v2.7.3 or later).
