...
<Update label="BloodHound" description="Enhancement" tags={["Cypher"]}>
  ...
  TODO: Update user guide (bhe.config.json) and refine this description.

  BloodHound now supports higher memory limits for user-submitted queries.

  This change raises the default user query memory ceiling from 1GB to 2GB, improving support for more demanding query workloads.
</Update>
...
<Update label="BloodHound" tags={["Fixed Issues"]}>
  ...
  - {/*BED-7088*/} Resolved an issue where the [Preview selectors](/reference/asset-isolation/preview-selectors) endpoint did not return the `source` enum for nodes, preventing clients from distinguishing seed, child, and parent results.
