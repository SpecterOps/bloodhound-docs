...
- **SharpHound (v2.6.7)**
  - New and Improved Features
    - SharpHound will now validate the currently installed version of .NET framework prior to execution.
    - Introduced support for Trust Account objects.
    - Added additional SMB Signing query transparancy in logging output.
  - Bug Fixes
...
- **New Attack Path primitive: HasTrustKeys** - The `HasTrustKeys` edge indicates Attack Paths which exist as a result of trusts configured between two domains. In the case where a domain is configured to hold the trust keys of the trust account, the trust keys can be leveraged to authenticate as the trust account and gain access to the trust account's domain.
...
- **_[BHE Only]_ Multiple Zones and Labels supported in early access** - BloodHound Enterprise customers may now create additional Zones and Labels for eventual support of the Privilege Zones feature. All BloodHound Enterprise customers may create additional zones, and may optionally choose to purchase the additional functionality to analyze those zones and track risk and remediation progress over time.
