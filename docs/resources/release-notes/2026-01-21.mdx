...
<Update label="BloodHound" description="Enhancement" tags={["API"]}>
  ...
  ## Source Kind Deactivation

  Maintain database integrity when removing data sources.
  
  When deleting an OpenGraph data source from the **Database Management** page, BloodHound now deactivates the `source_kind` instead of permanently removing it, preserving schema integrity while preventing future use. You can re-register and reactivate `source_kinds` as needed.
...
<Update label="BloodHound" description="Enhancement" tags={["Explore"]}>
  ...
  BloodHound now supports partial matching when searching for OpenGraph nodes in the **Search** tab on the **Explore** page, making it easier to locate specific nodes by name or object ID.
</Update>
...
<Update label="BloodHound" tags={["Fixed Issues"]}>
  ...
  {/*BED-6589*/} Resolved an issue where OpenGraph file ingestion reported _All files failed to ingest as JSON Content_ when files contained unresolved relationships. The [file upload API](/reference/collection-uploads/upload-file-to-job) now properly handles partial failures.
