...
<Update label="BloodHound" description="Enhancement" tags={["OpenGraph"]}>
  ...
  New [property-based edge matching](/opengraph/schema#edge-endpoint-matching) (`match_by: "property"`) enables hybrid edge creation by letting collectors identify external nodes using cross-system attributes, such as email, username, or hostname.
...
<Update label="BloodHound" description="Breaking Change" tags={["OpenGraph"]}>
  ...
  To keep node identity unambiguous for property-based endpoint matching workflows, `objectid` is now [reserved](/opengraph/schema#reserved-property-objectid) and may not be defined in a node's `properties` object.
