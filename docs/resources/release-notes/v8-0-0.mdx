...
Read the documentation: **LINK NEEDED HERE**
...
- **BloodHound (v8.0.0)**
  - New and Improved Features
    - Introducing BloodHound OpenGraph, enabling expansion and flexibilty of BloodHound to Attack Paths in any system!
...
- **New "Table" layout in Explore!** - Run a Cypher query that returns a bunch of objects (for example "Kerberoastable Users") and think it's silly to click through a bunch of nodes on a canvas? Us to! That's why we're adding in the Table layout to the Explore view. Put those nodes in a table, display whatever properties you like, export that data to a CSV. Attackers may think in graphs, but sometimes a list is a very helpful thing!
<Frame>
  <img src="/assets/table_view.png" alt="Explore table layout in BloodHound v8.0.0"/>
</Frame>
- **Inherited ACE tracking** - ACE inheritance is an incredibly powerful management tool within AD, but as the saying goes, "with great power comes great responsiblity to not create a bunch of Attack Paths" (it's something like that..). With BloodHound v8.0 (and SharpHound v2.5.4), you can now display the root and path of any inherited ACE in AD, enabling administrators to rapidly understand the root cause of, and to remove thousands of Attack Paths rapidly with a single change.
<Frame>
...
<Frame>
  <img src="/assets/data_deletion.png" alt="Targetted data deletion by type in BloodHound v8.0.0"/>
</Frame>
