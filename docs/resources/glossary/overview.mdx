...
import defExtensionDefinitionSchema from "/snippets/opengraph/extension-definition-schema.mdx";
...
Created by ingesting an OpenGraph data payload without an accompanying [extension definition schema](#extension-definition-schema). Generic graph data is limited to basic graph exploration and does not support advanced analysis features.
...
Created by ingesting an OpenGraph data payload with an accompanying [extension definition schema](#extension-definition-schema) that provides enhanced analysis and customization features in BloodHound.
