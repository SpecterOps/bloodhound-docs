...
- **Hygiene**—A tier-agnostic finding that identifies cleanup items not tied to a specific privilege tier. Examples include dangerous edges originating from broadly populated default groups. Hygiene findings are displayed separately on the **Posture** page rather than attributed to a specific tier.
