...
<Steps>
  ...
  Choose one of the following authentication methods for AzureHound Enterprise to connect to your Microsoft Entra ID and Azure environment:
    ...
    <Tabs>
      ...
      1. Run AzureHound Enterprise from the Azure resource that has this user-assigned managed identity assigned.

          <Note>If you run AzureHound Enterprise anywhere else, Azure cannot issue the managed identity token and authentication fails.</Note>
