...
<Steps>
  ...
  <Step title="Add Microsoft Graph API permissions">
    ...
    1. Search for the **Directory.Read.All** permission and check the box next to it.
        ...
        <Frame>
          <img
            src="/images/data_collectors/azurehound/directory-read-all.png"
            alt="A view of the Microsoft Graph API permission selection page with the Directory.Read.All permission highlighted."
            style={ { maxWidth: '70%' } }
          />
        </Frame>
    ...
    1. Search for the **RoleManagement.Read.All** permission and check the box next to it.
        ...
        <Frame>
          <img
            src="/images/data_collectors/azurehound/rolemanagement-read-all.png"
            alt="A view of the Microsoft Graph API permission selection page with the RoleManagement.Read.All permission highlighted."
            style={ { maxWidth: '70%' } }
          />
        </Frame>
...
We highly recommend using managed-identity-based authentication.

1. Create a [user-assigned managed identity](https://learn.microsoft.com/en-us/entra/identity/managed-identities-azure-resources/manage-user-assigned-managed-identities-azure-portal) in the Azure portal.

1. [Assign](https://learn.microsoft.com/en-us/entra/identity/managed-identities-azure-resources/how-to-configure-managed-identities#user-assigned-managed-identity) the managed identity to the host where AzureHound Enterprise will run.

1. [Configure](https://learn.microsoft.com/en-us/entra/workload-id/workload-identity-federation-config-app-trust-managed-identity) the app registration to use the managed identity.

### Certificate

This section requires you have authentication material.

If you do not already have a certificate created, follow the article [AzureHound Enterprise Local Configuration](/install-data-collector/install-azurehound/create-configuration) and then return back here.
...
**Editorial note: **This was different for me than the documented steps. I had to go to "Entra ID" > "Roles and administrators" > "Add assignments" (no Directory Readers role listed). From there, the only available role was "Cloud Application Administrator". I'm not sure if that's appropriate or not.

I'm still getting this error when running AzureHound on the VM:

```json
{
  "level": "error",
  "error": "failed to create new Azure client: map[error:map[code:Authorization_RequestDenied innerError:map[client-request-id:98051137-d6a9-43db-96b7-c3e546a9a3db date:2025-11-14T22:24:28 request-id:98051137-d6a9-43db-96b7-c3e546a9a3db] message:Insufficient privileges to complete the operation.]]",
  "time": "2025-11-14T22:24:28Z",
  "message": "encountered unrecoverable error"
}
```

That could be because I'm pointing to localhost instead of a publicly accessible BHE URL, but I wanted to note it here. Or it could be that I need to complete the steps in the [Grant "Reader" role on all Azure subscriptions](#grant-reader-role-on-all-azure-subscriptions) section below.
