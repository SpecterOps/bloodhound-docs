...
#### Integrated Windows Authentication (IWA) Example

If using Integrated Windows Authentication instead of API tokens, your `settings.json` would include the IWA-specific configuration fields:
...
```json
{
  ...
  "Resource": "https://bloodhound.example.com",
...
| ClientId | String | \[IWA Only\] The Client ID issued by ADFS during collector client creation. Used to identify SharpHound to the ADFS server. Required when `UseIntegratedWindowsAuthForADFS` is enabled. | null | "12345678-1234-1234-1234-123456789012" |
| Resource | String | \[IWA Only\] The resource identifier for the ADFS application. Provided during collector client creation for your BloodHound Enterprise tenant. Required when `UseIntegratedWindowsAuthForADFS` is enabled. | null | "https://bloodhound.example.com" |
| IssuerOverride | String | \[IWA Only\] Optional override for the ADFS token issuer address used during token validation. Use this if your ADFS token issuer differs from the well-known endpoint base URL. When not set, the issuer is derived from `ProviderWellKnown` by removing the "/.well-known/openid-configuration" suffix. | null | "https://adfs-custom.example.com" |
| UseIntegratedWindowsAuthForADFS | Boolean | \[IWA Only\] Enable Integrated Windows Authentication via ADFS. When set to `true`, SharpHound will authenticate using the service account's Windows credentials against the configured ADFS server. The `auth.json` file is not used when this is enabled. Requires `ProviderWellKnown`, `ClientId`, and `Resource` to be configured. | False | True |
