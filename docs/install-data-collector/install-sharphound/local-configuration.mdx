...
<Note>
  `ClientId` is the generated client ID that SharpHound sends when it requests a token from ADFS. It is separate from the optional **Custom Client Identifier** configured on the BloodHound Enterprise client. Do not add the custom identifier to `settings.json`; BloodHound uses it server-side when matching the identifier in the ADFS token.
</Note>
...
<ResponseField name="ClientId" type="string | null" pre={["IWA only"]}>
  Client ID generated by BloodHound during collector client creation. SharpHound sends this value to identify itself when requesting a token from the ADFS server. BloodHound uses the generated Client ID for token-to-client matching by default. Required when `UseIntegratedWindowsAuthForADFS` is enabled.
