...
| ForceLDAPSSL | Boolean | Controls SharpHound's primary LDAP connection behavior.<br/><br/>When set to `False`, SharpHound attempts LDAP over SSL first and can fall back to signed and sealed LDAP.<br/><br/>When set to `True`, SharpHound is configured to use LDAPS for its base LDAP configuration where possible, but some ancillary directory-related operations may still use other negotiation or resolution paths.<br/><br/>Do not use this setting as a guarantee that no TCP 389 traffic will occur in all scenarios. | `False` | `False` |
