...
<Warning>
In BloodHound Enterprise v9.0.0 and later, collector keys can expire based on your tenant API key expiration policy. When a collector key expires, SharpHound Enterprise stops authenticating and collections stop. Monitor the expiration date in **Manage Clients**, regenerate the key before it expires, and update auth.json on the host. For details, see [Configure API key expiration](/manage-bloodhound/auth/api-key-expiration).
</Warning>
