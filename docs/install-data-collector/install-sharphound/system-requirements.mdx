...
### Service Account Requirements for Integrated Windows Authentication (IWA)
...
* The service account must be configured in ADFS to support Windows authentication for SharpHound
  * See [Configuring ADFS for Integrated Windows Authentication](/install-data-collector/install-sharphound/configure-adfs-iwa) for detailed ADFS setup instructions
* The Client ID property must be registered in ADFS (provided during [collector client creation](/collect-data/enterprise-collection/create-collector))
* The Resource value must be specified in SharpHound's configuration (provided during client creation)
