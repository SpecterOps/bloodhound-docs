...
If SharpHound is configured to use Integrated Windows Authentication with your BloodHound Enterprise tenant, the following additional requirements apply:
...
* Local SharpHound configuration must include IWA-specific properties
    * `ProviderWellKnown` - The well-known endpoint URL for your ADFS server (e.g., `https://adfs.example.com/.well-known/openid-configuration`)
    * `ClientId` - The Client ID generated during collector client creation
    * `Resource` - Your BloodHound Enterprise tenant URL (e.g., `https://bloodhound.example.com`)
    * `UseIntegratedWindowsAuthForADFS` - Set to `true` to enable IWA authentication
