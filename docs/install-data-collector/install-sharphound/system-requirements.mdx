...
| [Local Groups/NTLM](/collect-data/permissions#local-groups--ntlm) | Local `Administrators` | Delegate Remote SAM and NTLM registry access with GPO/registry configuration |
...
| [DC Registry](/collect-data/permissions#dc-registry) | Local `Administrators` on domain controllers | Delegate access via GPO/registry configuration for required paths |
