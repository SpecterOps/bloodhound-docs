...
The standard SharpHound Enterprise installation process relies on UI-based interactions. If you need to install SharpHound Enterprise in an environment where interactive desktop sessions are unavailable or restricted, you can use the headless installation method.

Headless installation allows you to install SharpHound Enterprise without user interaction, making it suitable for automation tooling or other software deployment pipelines.

<Note>
  We recommend using a [Group Managed Service Account (gMSA)](/install-data-collector/install-sharphound/create-gmsa) for the SharpHound Enterprise service account rather than a regular AD user.
</Note>
...
<Steps>
  ...
  <Note>
    Be sure to specify the correct `<version>` of the installer in the command.
  </Note>
  </Step>
