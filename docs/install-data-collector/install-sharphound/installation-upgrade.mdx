...
<Steps>
  ...
  If you do not see the service, see [I don't see the SHDelegator Service](#i-dont-see-the-shdelegator-service).
  ...
  This initial startup fails, but it creates the configuration and log directory in the service account's user profile: `%AppData%\Roaming\BloodHoundEnterprise`.
  </Step>
  ...
  If the service does not start, see [The SharpHound Delegator Service won't start](#the-sharphound-delegator-service-wont-start).
...
<Steps>
  ...
  After installation, register the SharpHound Delegator service. Replace the placeholder values to match your environment.

  ```powershell
  $ServiceName = "SharpHoundDelegator"
  ...
  # Set the service account. For a gMSA, leave the password blank.
  sc.exe config $ServiceName obj= "$Username" password= "$Password"
  ```
  </Step>

  <Step title="Initialize the configuration directory">
  Start the service once. This startup fails, but it creates the configuration and log directory in the service account's user profile: `%AppData%\Roaming\BloodHoundEnterprise`.
  ...
  <Step title="Configure the service">
  Update `settings.json` and `auth.json` in `%AppData%\Roaming\BloodHoundEnterprise` as described in [SharpHound Enterprise Local Configuration](/install-data-collector/install-sharphound/local-configuration).
