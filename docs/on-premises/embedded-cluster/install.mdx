...
<Steps>
  ...
  <Step title="Download & install BloodHound Enterprise">
    ...
    1. From the portal, copy and run the command to download the installer archive on the Linux host.
       ...
       **Example:**
       ...
       ```bash
       curl -f "<url>" -H "Authorization: Bearer <token>" -o bloodhound-enterpriseLhC35jJua_CVAn52QHE_8yF0PzAa9zzl.tar.gz
       ```
  ...
  <Step title="Access BloodHound Enterprise">
    See the [Access BloodHound Enterprise](/on-premises/embedded-cluster/install#access-bloodhound-enterprise) section above for instructions on retrieving the initial admin password and logging in to the application.
  </Step>
</Steps>
