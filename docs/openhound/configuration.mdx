...
Configuring OpenHound works similarly for both the containerized service and the standalone CLI application. OpenHound uses DLT's [configuration management](https://dlthub.com/docs/general-usage/credentials/setup) system which allows you to set configuration parameters in the
`.dlt/config.toml` and `.dlt/secrets.toml` files, environment variables, or a combination of both. The following sections provide an overview of commonly used configuration parameters for OpenHound and how to set them up for your deployment.
...
The below sample configuration sets the global runtime, normalize and load values and overrides the extract worker count for the Okta and Github collectors.
...
The following log configuration options are supported by setting the parameters in `[runtime]` section or via environment
variables:
...
```toml Example for data writing overrides
[data_writer]
file_max_items=1000
...
**Note**: The data writer parameters parameters directly influence the performance and memory use of the collection/conversion pipeline.
Edges and nodes are processed in batches and the amount of processed items is determined by the data writer parameters. Setting these parameters too low can result in a large amount of small files and increased overhead with less memory usage,
while setting them too high can result in increased memory use and slower performance. It is recommended to experiment
...
```toml Example to enable more parallel workers for the Okta collector
[extract]
workers=5
...
The load phase is responsible for loading the converted OpenGraph files into the destination, which is either set a local file system or BloodHound Enterprise. 
