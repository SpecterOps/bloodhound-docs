...
### Kubernetes
A Helm chart is available for deploying OpenHound in a Kubernetes cluster. The chart is available in the [OpenHound GitHub repository](https://github.com/specterops/openhound).
...
To get started, create a `config.toml` and `secrets.toml` file with the necessary configuration parameters on the host machine.

Below is an example of a `docker-compose.yml` file to deploy OpenHound using Docker Compose, which mounts the configuration files and sets the necessary `DESTINATION__BLOODHOUNDENTERPRISE__COLLECTOR_NAME` variable to run each individual collector.
