...
via the `[sources.source.okta.credentials]` section of the secrets file or via environment variables using the `SOURCES__OKTA__CREDENTIALS__` prefix.

### Option 1: Service Application with JSON key file
| Parameter Name | Environment Variable            | Description                                                                        |
|----------------|---------------------------------|------------------------------------------------------------------------------------|
| `base_url`     | \{PREFIX\}_BASE_URL | The base URL of the Okta organization. For example: `https://spectoropspreview.oktapreview.com`. |
| `client_id`     | \{PREFIX\}_CLIENT_ID | The client ID of the Okta service application used to authenticate to the Okta API. |
| `private_key_path` | \{PREFIX\}_PRIVATE_KEY_PATH | The path to the private key (.json) used for authentication. |
...
| `base_url`     | \{PREFIX\}_BASE_URL | The base URL of the Okta organization. For example: `https://spectoropspreview.oktapreview.com`. |
| `client_id`     | \{PREFIX\}_CLIENT_ID | The client ID of the Okta service application used to authenticate to the Okta API. |
| `private_key_encoded` | \{PREFIX\}_PRIVATE_KEY_ENCODED | The private key (.json) encoded as a base64 string.  |

### Option 3: API Token (SSWS)
| Parameter Name | Environment Variable            | Description                                                                        |
|----------------|---------------------------------|------------------------------------------------------------------------------------|
| `base_url`     | \{PREFIX\}_BASE_URL | The base URL of the Okta organization. For example: `https://spectoropspreview.oktapreview.com`. |
| `token` | \{PREFIX\}_TOKEN | The API token (SSWS) used for authentication. |
