...
It collects data about organizations, users, teams, repositories, branches, roles, secrets, GitHub Apps, and other entities within a GitHub organization and represents them as [nodes](/opengraph/extensions/github/reference/schema#nodes) and [edges](/opengraph/extensions/github/reference/schema#edges) in BloodHound's graph database.
...
GitHub Collector is an open-source, read-only data collector. It requests only **read-only** permissions and never modifies your GitHub organization, repositories, or settings.
