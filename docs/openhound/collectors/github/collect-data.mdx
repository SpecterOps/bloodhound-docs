...
This page covers configuring the OpenHound GitHub collector for your GitHub organization or enterprise account. Use this page to choose an authentication method, set the required permissions, and configure the collector settings in the `secrets.toml` file or environment variables.
...
- One of the following authentication setups configured with the required permissions:
...
GitHub App authentication requires the following Read-only permissions.
...
<Note>
  These required permissions are only available for an [enterprise GitHub App installation](/openhound/collectors/github/configure-enterprise-app).
</Note>
...
<Tabs>
  <Tab title="Enterprise GitHub App">
    Use this option when you need enterprise-scoped collection, including GitHub Enterprise SSO data.
