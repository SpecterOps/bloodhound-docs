...
- Verify that an Enterprise Owner can create a classic PAT with the `read:enterprise` and `manage_runners:enterprise` scopes if you need enterprise SSO, SCIM, or self-hosted runner data.
...
<Steps>
  ...
  <Step title="Set the required scopes">
    ...
    - `read:enterprise`: Collects enterprise SSO and SCIM data.
    - `manage_runners:enterprise`: Collects enterprise self-hosted runner groups and runners.
  </Step>
  <Step title="Generate and copy the token">
    Click **Generate token**, then copy the token immediately.

    You add this token to the `pat_token` field in your `secrets.toml` file when you [configure the collector](/openhound/collectors/github/collect-data).
    <Warning>
...
- Grant the classic PAT only the `read:enterprise` and `manage_runners:enterprise` scopes.
