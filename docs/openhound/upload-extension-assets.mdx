...
| Upload fails with an authentication error | The BloodHound Enterprise URL or JWT is invalid or expired. | Update the `~/.dlt/config.toml` file with a valid URL, or the `~/.dlt/secrets.toml` file with a valid browser JWT, and rerun the command. |
