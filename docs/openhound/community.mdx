...
When you install OpenHound with `uv`, the CLI runs from an isolated tool environment that stays on your system until you replace or upgrade it. If a newer OpenHound release is available, upgrade the installed tool before you troubleshoot version-specific issues.
...
<Steps>
  <Step title="Upgrade the installed OpenHound tool">
    ...
    This updates the existing OpenHound tool environment to the latest compatible release.
  </Step>
  <Step title="Reinstall OpenHound with the collectors you use">
    If you use the built-in Okta, Jamf, or Github collectors, rerun the install command with the extras you want to keep in the tool environment:

    ```bash
    uv tool install "openhound[okta,jamf,github]"
    ```

    The core `openhound` package does not include collectors by default, so reinstalling with extras ensures the collector packages are available after the upgrade.
  </Step>
</Steps>
