...
custom collectors from the community. The configuration of OpenHound can be done through environment variables, configuration files or a
combination of both. Visit the [configuration](/openhound/configuration) page for more information on how to set up the configuration for OpenHound.
...
The `collect` subcommand is used to start the collection process. You can start the collecting data by specifying the collector name
and output directory. For example, to run the Okta collector and save the output in the `output` directory:

```bash
openhound collect okta ./output
```

OpenHound will then run the Okta collector and save the collected resources in the `./output/okta`. The collected
...
nodes and edges and save them as OpenGraph compatible json files that you can upload to BloodHound. The following command converts the collected Okta resources from the `./output/okta` directory and saves the OpenGraph files in the `./graph/okta` directory:
