...
In BloodHound Enterprise v9.0.0 and later, API key expiration is configurable for tenant administrators.
...
* API key rotation is manual. Users must create replacement keys before existing keys expire.

For configuration steps, see [Configure API key expiration](/manage-bloodhound/auth/api-key-expiration).
