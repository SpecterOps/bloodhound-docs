...
<Steps>
  ...
  <Step title="Install Docker Desktop">
    ...
    Follow the instructions on the [Docker Desktop](https://docs.docker.com/get-started/get-docker/) website to install Docker Desktop for your operating system.
  ...
  <Step title="Create installation directory">
    ...
    ```bash
    mkdir bhce-custom-install
    cd bhce-custom-install
    ```
  ...
  <Step title="Add configuration files">
    ...
    Copy the configuration files into the directory you created in the first step.
...
<Steps>
  ...
  <Step title="Start development environment">
    ...
    <Tip>See the source code [wiki](https://github.com/SpecterOps/BloodHound/wiki/Development#quick-start) on GitHub for more details about testing and debugging.</Tip>
...
PostgreSQL provides significant advantages over Neo4j as a backend, particularly in terms of speed. However, it has some drawbacks, the most notable being limited support for certain `Cypher` verbs and syntax.

For the latest information on supported Cypher syntax in PostgreSQL, see [Supported Cypher Syntax](/analyze-data/cypher-supported).
...
<Steps>
  ...
  <Step title="Remove the PostgreSQL graph driver">
    ...
    Delete the following line from the `bloodhound.config.json` file:
      ...
      <Note>If PostgreSQL becomes the default database in future versions of BHCE, you may need to add `"graph_driver": "Neo4j"` to your configuration file.</Note>
