...
SharpHound Enterprise collects [multiple data types](/collect-data/operations/data-retention) from Active Directory and its domain-joined systems. We recommend collecting all types for full risk identification and accurate risk assessment calculation.
...
1. Review the [SharpHound Enterprise System Requirements](/collect-data/sharphound/install/system-requirements) and [SharpHound Service Hardening Guidelines](/manage-bloodhound/securing-bloodhound-and-collectors/sharphound-hardening).
2. [Install and Upgrade SharpHound Enterprise](/collect-data/sharphound/install/install-and-upgrade).
3. To fully secure a domain, collect data from all other domains with a trust relationship to it (in- and outgoing trust). Configure SharpHound Enterprise for [Cross-Trust Collection](/collect-data/operations/cross-trust).
...
1. Review the [AzureHound Enterprise System Requirements and Deployment Process](/collect-data/azurehound/install/system-requirements).
2. [Configure Azure](/collect-data/azurehound/install/azure-configuration).
3. [Create your AzureHound configuration](/collect-data/azurehound/install/create-configuration).
4. [Deploy and maintain AzureHound](/collect-data/azurehound/install/installation-options).
...
1. [Scope Tier Zero for your environment](/core-concepts/tier-zero-members).
2. [Mark your environment's Tier Zero objects in BloodHound](/core-concepts/modifying-tier-zero).
