...
1. **Changes are saved**: Configuration is stored but not yet active
1. **Analysis is triggered**: Most changes automatically queue an analysis run (see table below)
1. **Analysis runs**: BloodHound re-evaluates all objects against the updated configuration
1. **Changes become visible**: Object assignments, metrics, and findings reflect the new configuration
...
| Update a zone (e.g., reorder, rename, toggle analysis) | <Icon icon="square-check" iconType="solid" color="#22c55e" /> | After analysis completes |
...
1. **Monitor analysis status**: Check your [tenant status](/collect-data/enterprise-collection/monitor#tenant-status) to see if analysis is running
1. **Wait for completion**: Analysis must finish before changes are reflected
1. **Review zone membership**: Navigate to the affected zone and verify that the expected objects appear
1. **Check findings**: Review the **Attack Paths** and **Posture** pages for any new findings or changes in existing findings related to the updated zones
