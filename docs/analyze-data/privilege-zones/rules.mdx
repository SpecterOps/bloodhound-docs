...
The process and screens for creating and editing rules is nearly the same for zones and labels. The primary difference is that [certification](/analyze-data/privilege-zones/certification) is an <Badge shape="rounded" size="sm" stroke color="purple">Enterprise Edition</Badge> feature available for zones only.
...
<Steps>
  ...
  <Step title="Configure rule details">
    ...
    * **Initial Objects**: Only the first set of objects directly matched by the rule are certified automatically (excludes objects added through [expansion](#rule-expansion), such as OUs and GPOs)
    * **All Objects**: Every object (including those tied to initial objects through expansion) is certified automatically
      ...
      **Rule type configuration details**
      ...
      <Tabs>
        ...
        <Tab title="Cypher">
          ...
          1. Click **Update Sample Results** above the Cypher query box. The first 200 results display in the **Sample Results** panel.
