      ...
      <Tabs>
        ...
        <Tab title="Cypher">
          ...
          1. Click **Run** below the Cypher query box. You must run the query before you can create the rule.
             ...
             <Note>
               When you need to match Tier Zero objects in a Cypher rule, use `MATCH (n:Tag_Tier_Zero)` rather than inspecting `system_tags` properties with patterns such as `coalesce(n.system_tags, [])`.

               You can combine the Tier Zero tag with another node label when needed, such as `MATCH (n:User:Tag_Tier_Zero) RETURN n`.
             </Note>
        </Tab>
      </Tabs>
