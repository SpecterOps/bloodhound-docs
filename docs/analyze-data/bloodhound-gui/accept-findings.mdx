...
* Logged in as a user role which is authorized to accept attack path impacted principals, see [Administering users and roles](/manage-bloodhound/auth/users-and-roles).
...
When a principal is accepted, it is hidden from the principal table in the attack path until you toggle the setting \`Accepted\`. The principal and it's edges will still visible in the Explore and Posture pages.
