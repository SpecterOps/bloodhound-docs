...
<Note>Environment Targeted Access Control (ETAC) can limit which environments a user can view in analysis pages. ETAC does not change who can accept findings, but it can limit which findings a scoped user can see. See [Configure ETAC](/manage-bloodhound/auth/environment-targeted-access-control).</Note>
