...
<Note>
This article provides an introduction to Cypher queries in BloodHound, including how to access prebuilt queries and manage saved queries. You can find more advanced information about writing custom Cypher queries in [placeholder advanced cypher page](https://bloodhound.readthedocs.io/en/latest/analyze-data/cypher-advanced.html).
</Note>
...
Selecting a query displays the Cypher syntax and automatically runs the query.
...
- **Save As**: Create a copy of an existing query with a new name and updated parameters.
...
- **Delete saved queries**: Remove saved queries that you no longer need.

### Import and export

- **Import a query from a JSON file**: Easily add new queries by dragging and dropping JSON files into the interface.
- **Export a saved query to a JSON file**: Share or back up your queries by exporting them in JSON format.
