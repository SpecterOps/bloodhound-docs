...
- **Filter queries**: Narrow down the list of queries by selecting one of the following options:
   ...
   - **Platforms**: Displays queries based on the platform that they target, such as Active Directory or Azure. The [data collectors](/opengraph/library) that you use influence this list. You can also filter to show only your saved queries.
...
<Steps>
  ...
  <Step title="Edit or delete your query">
    ...
    <Note>
      These options are availble to Admin users only when query sharing is **Set to Public**.
    </Note>
