...
- **Filter queries**: Narrow down the list of queries by selecting one of the following options:
   ...
   - **Platforms**: Displays queries based on the platform that they target, such as Active Directory or Azure. The data collectors that you use influence this list. You can also filter to show only your saved queries.
...
Creating a shared query enables collaboration with other users in your BloodHound environment. You can choose to make the query public or share it with specific users.

Shared queries of type "Personal" enable additional functionality, such as editing the query.
