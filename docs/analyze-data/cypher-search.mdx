...
This article describes how to use Cypher queries to extend the basic search functionality of BloodHound. BloodHound offers a variety of prebuilt queries to help you get started. You can also create and manage custom queries, import and export queries in JSON format, and filter queries by various criteria (e.g., platform, category, and source).
...
<Note>
This article provides an introduction to Cypher queries in BloodHound, including how to access prebuilt queries and manage custom queries. You can find more advanced information about Cypher syntax and capabilities in the [Supported Cypher Syntax](/analyze-data/cypher-supported) article.
</Note>
...
Inside BloodHound, you can explore prebuilt queries by clicking **Explore** > **Cypher** > **Saved Queries**. BloodHound displays prebuilt queries by default when you expand the **Saved Queries** section.
...
BloodHound provides several features to help you manage your Cypher queries effectively:

- **Search saved queries by name**: Quickly locate specific queries using the search bar.
- **Filter queries**: Narrow down your query list by platform (All, Active Directory, Azure, Saved), category, and source (All, Prebuilt, Personal, Shared).
- **Import queries from JSON**: Easily add new queries by dragging and dropping JSON files into the interface.
- **Export saved queries to JSON**: Share or back up your queries by exporting them in JSON format.
- **Save a query**: Store your custom queries for future use.
- **Use Save As**: Create a copy of an existing query with a new name.
- **Auto-run a selected query**: Execute a query automatically upon selection.
- **Delete queries**: Remove queries that are no longer needed.
- **Share a saved query**: Make your query public or share it with specific users for collaboration.
