...
<Note>
  `CREATE` requires `enable_cypher_mutations: true` in your BloodHound configuration file.
</Note>

### Create nodes

``` Cypher
CREATE (n:User {name: 'test-node', objectid: 'UUID-1234-567890'})
RETURN n
```
...
Returns an ordered list of all relationships in a matched path.
...
Returns the start node of a given relationship reference.

``` Cypher
match (:Computer)-[r:HasSession]->(:User)
return startNode(r)
```

### `endNode`

Returns the end node of a given relationship reference.
...
``` Cypher
match p = (:Computer)-[:MemberOf*..]->(g:Group)
return tail(nodes(p))
```
