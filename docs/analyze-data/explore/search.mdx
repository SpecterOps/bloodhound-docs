...
* **Add to/Remove from Tier Zero**—Mark or unmark the node as a member of the Tier Zero privilege zone.

  * Adding a node to Tier Zero runs analysis to update zone membership.
  
  * Removing a node requires manually editing the zone rule to remove the object.

* **Add to/Remove from Owned**—Mark or unmark the node as compromised. Adding is immediate; removing requires manually editing the label rule to remove the object.
