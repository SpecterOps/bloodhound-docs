...
BloodHound supports multiple data sources, including Active Directory, Azure (Entra ID), and other identity services through [OpenGraph](/opengraph/overview). The search methods return results for all data sources. The graph visualization options remain consistent regardless of which search method you use, allowing you to explore your data in a way that best suits your needs.
...
The **Search** tab allows you to quickly find specific nodes in the graph by name or object ID. As you type in the search text box, BloodHound automatically suggests nodes that match your search query. You can click on any of the suggestions to display that node in the graph.
...
<Note>Support for OpenGraph data coming soon.</Note>
