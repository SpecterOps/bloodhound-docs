...
<Note>
  To return Tier Zero objects in Explore, use `MATCH (n:Tag_Tier_Zero) RETURN n`.

  Avoid inspecting `system_tags` properties for this workflow. See [Search for Tier Zero objects](/analyze-data/explore/cypher-search#search-for-tier-zero-objects) for examples.
</Note>
