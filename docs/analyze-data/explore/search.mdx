...
<Note>
  Full-path highlighting is enabled by default. BloodHound stores this preference in the browser `localStorage` entry named `persistedState`, under `global.view.isExploreGraphHighlight`. The feature stays enabled unless that value is explicitly set to `false`.
