...
<Note>If your account uses Environment Targeted Access Control (ETAC), **Explore** is available only when your administrator enables **Explore access** for your user. Search results and graph data are limited to the environments you can access. See [Configure ETAC](/manage-bloodhound/auth/environment-targeted-access-control).</Note>
...
<Tip>When ETAC applies to your account, pathfinding returns only data from the environments you can access.</Tip>
