...
<Note>
  BloodHound supports all search methods for [structured](/opengraph/extensions/manage#structured-graphs) graphs. If you're exploring [generic](/opengraph/extensions/manage#generic-graphs) graphs, you can use the **Search** and **Cypher** methods only.
</Note>
...
The **Pathfinding** tab allows you to discover relationships between objects by finding paths from a start node through one or more destination nodes.

You can include a total of four destination nodes in a single search, which is useful for investigating potential attack paths across identity providers and cloud services in a single graph view.
...
* **Attack path analysis:** Identify potential compromise chains through selected objects
...
For example, you can find all paths from a user named "bob" to a group containing the name "domain admins" using the previously described [search](/analyze-data/explore/search#search) method for the start and destination nodes:
...
To build a path through multiple objects, select the nodes in the order that you want BloodHound to traverse them. BloodHound finds the shortest path between each consecutive pair and combines the results into one graph.
...
You can interact with nodes and edges in the graph to view detailed information about them in the [Entity panel](/analyze-data/explore/search#entity-panel). For nodes, you can right-click to perform more actions using the [context menu](/analyze-data/explore/search#context-menu).
...
* **Set as starting node**—Set the node as the first node in the **Pathfinding** tab and immediately draw a new graph showing paths through the current destination nodes
* **Set as ending node**—Set the node as the first destination in the **Pathfinding** tab and immediately draw a new graph showing paths from the current starting node through that destination and any remaining destination nodes
* **Add to/Remove from Tier Zero**—Mark or unmark the node as a member of the Tier Zero privilege zone. Adding automatically triggers analysis to tag the object; removing requires [manually editing](/analyze-data/privilege-zones/rules#edit-or-delete-a-rule) the zone rule to remove the object.
* **Add to/Remove from Owned**—Mark or unmark the node as compromised in the Privilege Zones page. Adding automatically triggers analysis to tag the object; removing requires [manually editing](/analyze-data/privilege-zones/rules#edit-or-delete-a-rule) the label rule to remove the object.
...
<Note>For built-in node and edge types (AD/AZ), BloodHound displays structured data organized into the accordions described below. For OpenGraph data, BloodHound displays all values from the [`properties`](/opengraph/developer/nodes) object as a flat list, without the structured accordions.</Note>
