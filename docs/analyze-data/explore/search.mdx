...
BloodHound clips long node labels by default to keep dense graphs readable. When you select a node, BloodHound shows that node's full label.

<Note>
  BloodHound stores the label clipping preference in the browser `localStorage` entry named `persistedState`, under `global.view.isExploreGraphLabelClip`.

  If you want to disable label clipping, open your browser's developer tools, run the following code in the console, and reload the page:

  ```javascript
  const state = JSON.parse(localStorage.getItem('persistedState'));
  state.global.view.isExploreGraphLabelClip = false;
  localStorage.setItem('persistedState', JSON.stringify(state));
  ```

  To restore the default behavior, set `state.global.view.isExploreGraphLabelClip = true`.
</Note>
...
<Note>
  BloodHound stores the full-path highlighting preference in the browser `localStorage` entry named `persistedState`, under `global.view.isExploreGraphHighlight`.

  If you want to disable full-path highlighting, open your browser's developer tools, run the following code in the console, and reload the page:

  ```javascript
  const state = JSON.parse(localStorage.getItem('persistedState'));
  state.global.view.isExploreGraphHighlight = false;
  localStorage.setItem('persistedState', JSON.stringify(state));
  ```

  To restore the default behavior, set `state.global.view.isExploreGraphHighlight = true`.
</Note>
