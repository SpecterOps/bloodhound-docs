...
## Search for Tier Zero objects

When you need to return Tier Zero objects in Cypher, match on the Tier Zero tag label directly:
...
This is the preferred pattern for Explore workflows, saved queries, and Cypher-based Privilege Zone rules.

If you need to restrict the results to a specific node type, combine labels in the same pattern:
