...
| **Status** | The current status of the finding: **Open**, **Accepted**, **Remediated**, **Deprecated**, or **Orphaned**. See [Finding status lifecycle](#finding-status-lifecycle) for definitions and the actions that move a finding between statuses. |
...
flowchart LR
    ...
    subgraph archived["System-archived outcomes"]
        ...
        orphaned["Orphaned<br/>Environment archived or removed"]
...
| **Orphaned** | `orphaned` | The environment associated with the finding was archived or removed and is no longer collected, so BloodHound archived the finding. | Yes |
...
<Note>
  **Open** and **Accepted** are not stored as a status value. BloodHound derives them from the finding's acceptance state. **Remediated**, **Orphaned**, and **Deprecated** are stored statuses that also mark a finding as archived.
</Note>

### How BloodHound determines status

BloodHound evaluates a finding's status in the following order:

1. If the finding has a stored lifecycle status (**Remediated**, **Orphaned**, or **Deprecated**) BloodHound displays that status. These findings are archived.
1. Otherwise, if the finding has an acceptance that has not yet expired, BloodHound displays **Accepted**.
1. Otherwise, BloodHound displays **Open**.

Because BloodHound derives **Accepted** and **Open** from the acceptance expiration, an accepted finding automatically returns to **Open** when its acceptance period expires.
