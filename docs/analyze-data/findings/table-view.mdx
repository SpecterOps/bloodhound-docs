...
| Column | Description |
| --- | --- |
| **Risk** | When [Findings Prioritization](#prioritize-findings) is enabled on the **Early Access Features** page, displays the finding's rank. Rank **1** is the highest priority. The column replaces **Severity** in the table and uses severity-specific colors. |
...
A higher-ranked finding represents a stronger combination of underlying factors present in the enterprise's graph data.
