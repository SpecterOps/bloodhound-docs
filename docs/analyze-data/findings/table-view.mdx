...
$$
\text{Prioritization score} =
\text{Impact count} \times
\text{Exposure count} \times
\text{Starting objects}
$$
...
<Note>
  Severity remains visible as a separate risk signal. **Findings Prioritization** supplements severity; it does not redefine it.
</Note>
...
<Note>
  If BloodHound has not calculated a rank for a finding, the **Risk** column displays `-`.
</Note>
