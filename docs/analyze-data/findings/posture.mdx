...
For foundational concepts, see [Understand Findings and Remediation](/analyze-data/findings/attack-paths).

<Frame>
  <img src="/images/posture_page/page-filter.png" alt="A view of the Posture page filter options" />
</Frame>

## Interpret findings, attack paths, and change

Use these terms consistently when you read the dashboard:

* **Attack Paths** count the routes that can be abused.
* **Findings** count specific risk instances.

These values are related, but not identical. One finding can include multiple attack paths, so attack path counts are often higher than finding counts.

The **Change** column shows net change during the selected date range. A positive value means the count increased over that period. A negative value means the count decreased.

For meaningful trend comparisons, use the same filter scope and similar time windows across reviews.
