...
The **Posture** page is the primary reporting dashboard that helps you understand and track your organization's security posture over time. It aggregates data from findings to provide insights about where the biggest risks originate and how your remediation efforts are reducing risk.

Filter the view by environment, tier zero, hygiene, and time range to assess your overall risk.

<Note>
Hygiene refers to findings such as dangerous edges from broadly populated default groups, and are not attributed to any specific privilege tier.
</Note>

The time range filter allows you to compare posture trends between analysis runs. For example, you can compare the current state of your environment to a previous point in time to see how your risk posture has changed.
...
* **Attack Paths**—Represent the routes that can be abused to reach a privileged asset (such as a domain controller). Each attack path is made up of one or more edges between principals.
* **Findings**—Represent specific principals and relationships between principals that can be abused to reach a privileged asset. Each finding can be made up of one or more attack paths. This is why attack path counts and finding counts can differ.
...
* **Tier Zero Objects** - This graph represents the trend in the total number of objects in the Tier Zero Privilege Zone within the selected filter parameters over time.
  ...
  <Frame>
    <img src="/images/posture_page/tier-zero-graph.png" alt="A view of the Tier Zero Objects graph on the Posture page" />
  </Frame>
