...
<Note>If your account uses Environment Targeted Access Control (ETAC), the **Posture** page shows data only for the environments you can access. ETAC does not replace filters such as domain, privilege zone, or time range. See [Configure ETAC](/manage-bloodhound/auth/environment-targeted-access-control).</Note>
