...
The view may be filtered on a specific environment, tenant, or all of each and a duration over which to view the overall risk.
...
BloodHound Enterprise calculates the severity rating based on the percentage of users and computers that can abuse the Attack Path. 
For example, a CRITICAL attack path is one that is abuseable by 95% - 100% of all users and computers in the environment. The different severity rankings and exposure levels are:
...
This is a series of visualizations contains graphs that show posture over time.
...
Completeness is calculated as the number of computers BloodHound was able to scan for Sessions and Groups as a percentage of total computers.
