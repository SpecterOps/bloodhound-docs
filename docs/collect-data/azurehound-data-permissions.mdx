...
AzureHound collects the Entra ID and Azure Resource Manager data that BloodHound uses to model Entra DS. Ensure the AzureHound collection includes both Entra ID (`az-ad`) and Azure Resource Manager (`az-rm`) data. The `az-rm` scope collects each accessible [`AZEntraDS`](/resources/nodes/az-entra-ds) resource and role assignments scoped directly to that resource.

Collecting an Entra DS resource requires permissions which are included in the downloadable AzureHound Reader role above.

AzureHound does not collect the users, groups, computers, ACLs, or other Active Directory objects in the managed domain. Use [SharpHound to collect the Entra DS managed domain](/collect-data/sharphound-data-permissions#microsoft-entra-domain-services).
