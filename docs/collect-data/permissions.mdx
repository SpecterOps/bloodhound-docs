...
<Note>[Least-Privileged Collection](/collect-data/enterprise-collection/least-privileged-collection) is possible for most BloodHound data types.</Note>

By default, computers beginning with Windows 10 version 1607 and Windows Server 2016 require Administrator access to perform RemoteSAM operations. This setting may be overridden with Group Policy for least-privilege collection. This is the only permission required by SharpHound for which Microsoft supports modification.
