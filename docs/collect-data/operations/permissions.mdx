...
In BloodHound Enterprise you can start scans for different data types via a [collection schedule](/collect-data/operations/collection-schedule) or an [on-demand scan](/collect-data/operations/on-demand-scan).
With BloodHound Community Edition, you run scans by [running the executable itself](/collect-data/community/sharphound).
