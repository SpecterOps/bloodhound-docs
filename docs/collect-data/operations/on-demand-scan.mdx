...
* Creation of one SharpHound Enterprise client, see [Create a SharpHound Enterprise collector client](/collect-data/operations/create-collector)
...
4. In the *On Demand Scan* window, configure the scan:
   1. **Data**: The type of data the schedule collects, see [SharpHound Data Collection and Permissions](/collect-data/operations/permissions)
   2. **Advanced Options**: See the [Scanning section in the article SharpHound Enterprise Tenant Configuration](/collect-data/sharphound/install/configure-tenant)
...
After completion, see the job's status in the *Finished Jobs Log* by clicking ⚙️ → **Administration**, and in the top left side, click **Finished Jobs** **Log**\_. \_The columns **Status** and **Status Message** report on the job's completion.
