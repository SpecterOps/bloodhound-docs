...
- BloodHound Enterprise scan types can be started with a [collection schedule](/collect-data/enterprise-collection/collection-schedule) or an [on-demand scan](/collect-data/enterprise-collection/on-demand-scan)
...
**Least-Privileged Option:** For information on how to collect all objects with least privilege, see [Least-Privileged Collection - AD Structure Data](/collect-data/enterprise-collection/least-privileged-collection#ad-structure-data).
...
SharpHound collects the Active Directory objects materialized in a Microsoft Entra Domain Services (Entra DS) managed domain through normal AD Structure collection; there is no separate Entra DS collection method.

For synchronized users and groups, SharpHound collects the `msDS-aadObjectId` LDAP attribute and BloodHound stores it as the `aadobjectid` property. BloodHound uses this property to correlate the managed-domain objects with their corresponding Entra ID identities.

SharpHound does not collect the Entra ID or Azure Resource Manager control planes. Use [AzureHound to collect the complementary Entra DS data](/collect-data/azurehound-data-permissions#microsoft-entra-domain-services).
...
**Least-Privileged Option:** Delegation is possible via Group Policy or registry modifications, see [Least-Privileged Collection - NTLM Relay Registry Paths](/collect-data/enterprise-collection/least-privileged-collection#ntlm-relay-registry-paths).
...
**Additional Data Sources:** Two additional types of data can enhance the findings - [DC Registry](/collect-data/sharphound-data-permissions#dc-registry) and [CA Registry](/collect-data/sharphound-data-permissions#ca-registry).
