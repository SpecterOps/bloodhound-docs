...
<Steps>
  <Step title="Download SharpHound CE">
    Download the latest version of SharpHound CE from [GitHub](https://github.com/SpecterOps/SharpHound/releases/latest).
  </Step>
...
<Steps>
  ...
  <Step title="Upload collected data">
    ...
    <Note>After the upload completes, you can [verify the status](#view-file-ingest-logs) in the **Details** panel. Click the file ingest **ID** in the table to open the **Details** panel on the right side of the screen.</Note>
  </Step>
</Steps>

### View File Ingest Logs

You can monitor the ad-hoc file ingestion to ensure successful data processing. It shows the status of each file ingest operation, which can be helpful for viewing data upload history or troubleshooting data ingestion issues.
...
To view the **File Ingest** logs:

<Steps>
  <Step title="Navigate to the File Ingest page">
  In the left menu, click **Administration** > **File Ingest**.
  </Step>

  <Step title="Open the Details panel">
  Click on the specific file ingest **ID** in the table to open the **Details** panel.

  <Tip>You can also click the <Icon icon="filter" /> icon to filter by status, data range, and user.</Tip>
  </Step>

  <Step title="Review the log entries">
  In the **Details** panel, review the log entries for the selected file ingest. Look for any errors or warnings that may indicate issues during the ingestion process.
...
After ingest and analysis is complete, BloodHound Enterprise presents a comprehensive report with actionable recommendations on the [**Attack Paths**](/analyze-data/accept-findings) page.
