...
| **Analyzing** | Data collection is complete and analysis is running. Begins immediately after successful ingestion or on demand. |
...
| **Data Collected** | The [type of data](/collect-data/permissions) that the client is configured to collect |

<Note>When a collector client has a [Collection Schedule](/collect-data/enterprise-collection/collection-schedule) configured, jobs on the **Finished Jobs Log** page display an *Analyzing* status while the tenant status remains *Idle*. The tenant status changes to *Ingesting* when the next scheduled collection time is reached. Analysis begins automatically after the scheduled collection successfully finishes running.</Note>
...
When you perform an [ad-hoc data collection](/collect-data/enterprise-collection/ad-hoc-collection) by uploading a SharpHound output .zip file, ingestion process details are logged on the **File Ingest** page.
