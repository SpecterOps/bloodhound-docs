...
This guide explains how to configure a data collector client to run on a schedule. Administrators should use it when deploying a new client or adding an additional schedule to an existing client.
...
<Steps>
  ...
  On the client that you want to schedule, click the <Icon icon="bars" /> icon, and select **Edit Client**.
...
After the next schedule, see the job's status in the _Finished Jobs Log_ by clicking **Administration** > **Finished Jobs Log**. The **Status** and **Status Message** columns report on the job's completion.

<Frame>
  <img
    src="/images/data_collectors/finished-jobs-log-status.png"
    alt="A view of the Finished Jobs Log page showing job status and status message"
  />
</Frame>
