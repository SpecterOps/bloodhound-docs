...
<Note>See [SharpHound Enterprise System Requirements](/install-data-collector/install-sharphound/system-requirements), [AzureHound Enterprise System Requirements](/install-data-collector/install-azurehound/system-requirements), or [OpenHound server/container requirements](/openhound/enterprise#server%2Fcontainer-requirements) for collector-specific requirements.</Note>
...
<Steps>
  ...
  <Step title="Create the client">
    ...
    1. Complete the required fields:
        ...
        | **Issuer Address** | Yes<br/><br/>(*IWA only*) | The ADFS well-known endpoint URL, typically: `https://adfs.example.com/.well-known/openid-configuration` |
        ...
        | **Custom Client Identifier** | No<br/><br/>(*IWA only*) | An optional value that BloodHound Enterprise uses to associate the ADFS bearer token with this client when the token identifies the client with a value other than the generated Client ID. The value must exactly match the configured token claim and be unique across clients. Leave blank to use the generated Client ID. |
        ...
        <Tabs>
          ...
          The following screenshot shows the client creation form when **Integrated Windows Authentication** is selected.

          Note the additional required **Issuer Address** field and optional **Issuer Address Override** and **Custom Client Identifier** fields.
            <Frame>
              <img
    ...
    <Tabs>
      ...
      <Tab title="Integrated Windows Authentication">
        ...
        <Note>
          ...
          If you entered a Custom Client Identifier, record it for the ADFS claim-rule configuration. The value must be included in the ADFS token's client-identifier claim; do not add it to SharpHound's `settings.json` file.
        </Note>
...
OpenHound collector clients use API token-based authentication. When creating an OpenHound collector client, you must save the generated **Token ID** and **Token** before closing the dialog. You use these credentials to [configure](/openhound/configuration#enterprise-2) the OpenHound `secrets.toml` file.
...
<Steps>
  ...
  <Note>
    The token information is required to [configure](/openhound/configuration#enterprise-2) the OpenHound `secrets.toml` file.
  </Note>
...
- OpenHound clients:
  - Use the **Token ID** and **Token** to [configure](/openhound/configuration#enterprise-2) the OpenHound `secrets.toml` file.
