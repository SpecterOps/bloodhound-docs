...
<Warning>
The collector key is only shown once. If you lose it, you must regenerate it from the **Manage Clients** page. In BloodHound Enterprise v9.0.0 and later, collector keys can expire based on your tenant API key expiration policy. Monitor expiration dates in **Manage Clients** and regenerate keys before they expire to prevent collection from stopping. For details, see [Configure API key expiration](/manage-bloodhound/auth/api-key-expiration).
</Warning>
