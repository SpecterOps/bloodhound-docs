...
5.  The pop-up window _Client Token Info_ will appear; follow the instructions in it – save the API token and Token ID before clicking **CLOSE**. Both values are required for authentication and will only be shown once.
<Frame>
  <img src="/assets/image-73.png" alt=""/>
</Frame>

<Warning>
The API token and Token ID are only shown once. If you lose either value, you must regenerate them from the **Manage Clients** page.
</Warning>
