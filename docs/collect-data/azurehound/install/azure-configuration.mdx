...
5. In the **Overview** menu, copy the **Application (client) ID** and **Directory (tenant) ID** to be used later in [AzureHound Enterprise Local Configuration](/collect-data/azurehound/install/create-configuration).
   <Frame>
...
We highly recommend using certificate-based authentication. If you do not already have a certificate created, follow the article [AzureHound Enterprise Local Configuration](/collect-data/azurehound/install/create-configuration) and then return back here.
...
4. Locate the **cert.pem** file created during AzureHound setup (either on your own, or utilizing the instructions at [AzureHound Enterprise Local Configuration](/collect-data/azurehound/install/create-configuration)).
...
15. Continue to [Run and Upgrade AzureHound (Windows, Docker, or Kubernetes)](/collect-data/azurehound/install/installation-options)
