...
A regular, non-privileged user can run Shaprhound and collect a significant amount of information Active Directory, but using an account that has certain permissions enables the SharpHound executable to collect all of the possible pertinent information from Active Directory for the best possible analysis by BloodHound. See [SharpHound Data and Permissions](/collect-data/sharphound-data-permissions) for an overview.

At this time there are two main paths that could give the SharpHound gMSA account the permissions needed to collect all required Active Directory information:
