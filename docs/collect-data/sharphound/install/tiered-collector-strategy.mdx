---
title: Deploy a Tiered Collector Strategy
---
...
* Creation of one BloodHound collector server for each tier, see [SharpHound Enterprise System Requirements](/collect-data/sharphound/install/system-requirements)
  * Tip: In BloodHound, mark the Tier Zero collector server as Tier Zero, see [Modifying Tier Zero](/core-concepts/modifying-tier-zero)
* Creation of one SharpHound Enterprise service account for each tier, see [Create a gMSA for use with SharpHound Enterprise](/collect-data/sharphound/install/configure-gmsa)
  * Each service account must have collection permission on all systems in the service account's tier - local `Administrators` group membership or [Least-Privileged Collection](/collect-data/operations/least-privileged-collection) permissions.
  ...
  * Tip: In BloodHound, mark the Tier Zero service account as Tier Zero, see [Modifying Tier Zero](/core-concepts/modifying-tier-zero)
...
1. Follow the article [Create a SharpHound Enterprise collector client](/collect-data/operations/create-collector)
...
2. Install the collector client on the dedicated Tier Zero BloodHound collector server, using the dedicated Tier Zero SharpHound Enterprise service account. [See Install and Upgrade SharpHound Enterprise](/collect-data/sharphound/install/install-and-upgrade).
...
1. On the Tier Zero collector client, create a new collection schedule, see [Create a data collection schedule](/collect-data/operations/collection-schedule)
