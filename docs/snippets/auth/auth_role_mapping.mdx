...
A role is applied when the role key is present and its value is a properly formatted BloodHound role.
...
Only one role will be accepted in the role claim. If more than one role is provided, the provider default role will apply.