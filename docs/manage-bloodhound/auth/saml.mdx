...
1.  Configure the IDP for BloodHound.
    ...
    - [SAML: Okta Configuration](/manage-bloodhound/auth/saml-okta)
...
1.  Create the SAML Configuration in BloodHound.
    ...
    For Entra ID, set **Assignment required?** to **Yes** and assign only the required users and groups. See [SAML: Entra ID Configuration](/manage-bloodhound/auth/saml-entra-id).
    </Note>

1.  [Create new users or modify existing users](/manage-bloodhound/auth/users-and-roles) using the UI or via the newly created SAML provider.
