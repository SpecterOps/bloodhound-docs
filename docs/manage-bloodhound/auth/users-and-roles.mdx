...
In BloodHound Enterprise, Environment Targeted Access Control (ETAC) can further limit which environments **User** and **Read-only** roles can access. ETAC does not change the baseline permissions in the role matrix below. Instead, it limits where those permissions apply and whether eligible users can open **Explore**. For ETAC workflow details, see [Configure ETAC](/manage-bloodhound/auth/environment-targeted-access-control).
