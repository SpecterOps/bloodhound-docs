...
1.  Determine the Identity Provider (IDP) name you will use for the OIDC configuration.
    ...
    The same value must be configured in both the IDP and BloodHound.
...
1. Follow the [Okta documentation](https://help.okta.com/oie/en-us/content/topics/apps/apps_app_integration_wizard_oidc.htm) to create a new application.

    <Note>Set your application type to **SPA** (Single Page Application) or **Native**.</Note>

1. From your Okta application, note the following values:

    - **Client ID**
    - **Issuer URL**

    <Tip>You'll use the **Client ID** from the Okta _Client Credentials_ and the **Issuer URL** from the Okta _Authorization Server_ when you [configure BloodHound](/manage-bloodhound/auth/oidc#configure-bloodhound).</Tip>

1. Add the BloodHound callback URL to your Okta application configuration using the following format:

    ```
    https://{domainname}/api/v2/sso/{chosenProviderName}/callback
    ```
...
To create the required custom claims in Okta:
