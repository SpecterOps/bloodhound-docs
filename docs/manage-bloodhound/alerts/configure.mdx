...
If you are new to **Alerts** in BloodHound Enterprise, start with the [Overview](/manage-bloodhound/alerts/overview) to learn how delivery, rules, and event history fit together. Then return here when you are ready to configure a webhook.
...
Webhooks in BloodHound Enterprise are reusable destinations. Multiple rules can send alert events to the same webhook.

<Note>
  We'll cover how to configure rules that connect event types to webhook destinations later in this procedure. For now, focus on creating a webhook and verifying that your receiver can accept events.
</Note>
...
Use **Run Test** from the webhook's actions menu before you create a production rule. The test sends a sample alert event to the webhook destination.
...
Rules allow you to define which event types BloodHound Enterprise sends to which webhook destinations. When BloodHound Enterprise generates a matching event, it sends the alert event to the selected webhook destination.
...
| **Run Test** | Sends a sample alert event to the webhook destination. |
| **Regenerate Secret** | Replaces the current HMAC secret. Update your receiver before you resume normal delivery. |
| **Disable Webhook** | Temporarily stops every rule from sending to this webhook destination without deleting its configuration.<br/><br/>Use it while you repair, reconfigure, or review the receiving system. Select **Enable Webhook** to resume delivery. Re-enabling a webhook resets its delivery health. |
...
| **Disable Notification** | Temporarily stops this rule from delivering events without affecting other rules that use the same webhook.<br/><br/>Use it to suppress expected alert events, such as collector maintenance. Select **Enable Notification** to resume delivery. |
