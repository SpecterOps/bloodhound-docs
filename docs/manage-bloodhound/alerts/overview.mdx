...
Alerts separate events from destinations, allowing you to manage webhook endpoints and the rules that use them independently.
...
1. When an event is triggered in BloodHound Enterprise, it finds enabled matching rules, queues a delivery attempt for each configured webhook, sends the signed event, and records the outcome on the **Event History** page.
