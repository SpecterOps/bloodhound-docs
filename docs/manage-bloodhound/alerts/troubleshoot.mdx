...
BloodHound Enterprise sends (at most) one Collector Offline event for each collector during a seven-day cooldown. It does not send a recovery event when that collector checks in again.
...
Use **Run Test** from the webhook actions menu to send a representative event without waiting for a collector to become offline.
