...
<ResponseField name="is_display_kind" type="boolean">
  ...
  - When `true`: BloodHound can use this kind's `icon` and `color`. If a node has multiple display kinds, BloodHound uses the first one in the `kinds` array.
  - When `false`: The kind can still appear on nodes, but BloodHound ignores it when selecting custom icon and color.

  <Note>
    If custom styling has already been stored for this kind, changing `is_display_kind` to `false` does not remove the existing icon or color mapping. Delete the custom icon definition through the API to remove it.
  </Note>

  See [Schema Behavior Examples](/opengraph/developer/schema-behavior-examples#set-display-kinds-for-node-styling) for a comparison of how display kinds affect node appearance in BloodHound.
</ResponseField>
...
<ResponseField name="environment_kind" type="string" required>
  ...
  See [Schema Behavior Examples](/opengraph/developer/schema-behavior-examples#define-environments-for-selection-and-grouping) for an example of how BloodHound uses environment definitions in the UI.
</ResponseField>
...
<ResponseField name="principal_kinds" type="array" required>
  ...
  See [Schema Behavior Examples](/opengraph/developer/schema-behavior-examples#define-principal-kinds-for-findings-and-metrics) for an example of how principal definitions affect analysis scope.
</ResponseField>
...
<ResponseField name="remediation" type="object">
  ...
  See [Schema Behavior Examples](/opengraph/developer/schema-behavior-examples#add-remediation-text-to-relationship-findings) for an example of how finding and remediation text appears in BloodHound.
</ResponseField>
