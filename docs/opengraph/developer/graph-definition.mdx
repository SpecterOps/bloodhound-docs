...
<ResponseField name="name" type="string" required>
  Unique node kind identifier. Must follow [namespacing](/opengraph/developer/graph-definition#namespacing) rules. Nodes in the data payload must match at least one of the `kinds` defined here.
...
<ResponseField name="name" type="string" required>
  Unique relationship kind identifier. Must follow [namespacing](/opengraph/developer/graph-definition#namespacing) rules. Edges in the data payload must match at least one of the `kinds` defined here.
</ResponseField>
<ResponseField name="description" type="string">
...
Environments are platform-specific groupings of nodes that BloodHound Enterprise uses to scope findings and metrics. Each environment definition specifies which node kinds represent <Tooltip tip="A type of object that can authenticate and be assigned permissions within an environment.">principals</Tooltip> within each environment.
...
<ResponseField name="source_kind" type="string" required>
  Source kind that associates this environment definition with a specific platform for environment organization and selection.

  For findings and metrics in BloodHound Enterprise, nodes in this environment must include a `source_kind`. This value can be applied payload-wide with [`metadata.source_kind`](/opengraph/developer/metadata#param-source-kind) or manually in the [`node_kinds`](/opengraph/developer/nodes#param-kinds) array in the data payload.
</ResponseField>
<ResponseField name="principal_kinds" type="array" required>
  Node kinds defined by this extension/schema that BloodHound should treat as principals. Must match a `node_kinds.name` defined in this schema.
...
<Tip>
  **Best practice**
  ...
  If a finding `name` would match its associated `relationship_kind`, include `Finding` in the name to distinguish them.
...
<ResponseField name="relationship_kind" type="string" required>
  Relationship kind that contributes to this finding when matching edges are present and the edge crosses a Privilege Zone boundary. Must match a `relationship_kinds.name` defined in this schema.
</ResponseField>
