...
<ResponseField name="name" type="string" required>
  Unique node kind identifier. Must follow [namespacing](/opengraph/developer/graph-definition#namespacing) rules. For each node in a data payload, at least one value in its `kinds` array must match a `node_kinds.name` defined in the extension definition schema.
...
<ResponseField name="name" type="string" required>
  Unique relationship kind identifier. Must follow [namespacing](/opengraph/developer/graph-definition#namespacing) rules. For each edge in a data payload, `kind` must match a `relationship_kinds.name` defined in the extension definition schema.
</ResponseField>
<ResponseField name="description" type="string">
...
Environments are platform-specific groupings of nodes that BloodHound Enterprise uses to scope findings and metrics. Each environment definition specifies which node kinds represent <Tooltip tip="An object type that can authenticate, be assigned permissions within an environment, and serve as a basis for risk metric calculations.">principals</Tooltip> within each environment.
...
<ResponseField name="principal_kinds" type="array" required>
  Node kinds defined by this extension/schema that BloodHound should treat as <Tooltip tip="An object type that can authenticate, be assigned permissions within an environment, and serve as a basis for risk metric calculations.">principals</Tooltip>. Must match a `node_kinds.name` defined in this schema.
