...
<ResponseField name="is_display_kind" type="boolean">
  ...
  BloodHound uses display kinds to decide which `icon` and `color` to show for matching nodes. Set this field to `true` when this kind should be eligible to represent the node visually. Set it to `false` when the kind is only used for classification, analysis, or other schema logic and should not control the node's appearance.

  - When `true`: BloodHound can use this kind's `icon` and `color` for matching nodes in the graph. If a node has multiple display kinds, BloodHound uses the first one in the `kinds` array to determine the node's visual representation.
  - When `false`: The kind can still appear on nodes, but it does not drive custom icon or color selection.
