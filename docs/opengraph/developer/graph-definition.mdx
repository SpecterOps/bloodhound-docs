...
| `markdown.content` | Markdown rendered in the section body. Required when `markdown` is present. You can use static Markdown or a Go template that renders values for the selected entity. See [Create dynamic Entity Panel content](/opengraph/developer/dynamic-entity-panel-content). |
...
| Field(s) | Where |
| --- | --- |
| `info.<section>.markdown.content` | `node_kinds` and `relationship_kinds` arrays. This content can also use a [dynamic Entity Panel template](/opengraph/developer/dynamic-entity-panel-content). |
...
Only `info.<section>.markdown.content` supports dynamic templates. BloodHound treats Markdown in `relationship_findings.remediation` as literal Markdown.

When you upload an extension definition schema, BloodHound also parses every `info.<section>.markdown.content` value in the `node_kinds` and `relationship_kinds` arrays as a Go template. An invalid template, including an unknown helper function, causes the extension upload to fail.

For template syntax, contexts, supported authoring patterns, and testing guidance, see [Create dynamic Entity Panel content](/opengraph/developer/dynamic-entity-panel-content).
