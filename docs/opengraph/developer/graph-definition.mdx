...
Only `info.<section>.markdown.content` supports dynamic templates.

When you upload an extension definition schema, BloodHound parses every `info.<section>.markdown.content` value in the `node_kinds` and `relationship_kinds` arrays as a Go template. An invalid template, including an unknown helper function, causes the extension upload to fail.
