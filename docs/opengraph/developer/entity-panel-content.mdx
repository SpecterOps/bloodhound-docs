...
1. Fixed query parameters that select the page or view.
1. Dynamic parameter values rendered from the node or relationship context.
...
1. Build the complete Cypher query with `printf`.
1. Encode the complete query with `b64enc` and `urlquery`.
1. Use the encoded value as the `cypherSearch` parameter in an Explore link.
...
1. Decode and test any generated Cypher link.
