...
An opening `if` action requires one or more arguments that evaluate to boolean values. Use conditional checks to produce those values. The `else` action does not take arguments; it renders the alternate branch when the `if` condition is false. For example:
...
### Conditional checks and null handling
