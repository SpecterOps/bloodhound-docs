...
- **Generic graph data** refers to OpenGraph payloads that follow the minimum node and edge schemas described on this page.
- **Structured graph data** refers to OpenGraph payloads that use an extension definition schema outside the payload itself.
...
This page focuses on the JSON requirements for generic graph data.
...
When ingest completes, you can [search](/analyze-data/explore/search) OpenGraph data. Supported search methods depend on whether the data is structured or generic:

| Search method | Structured | Generic |
| --- | --- | --- |
