...
All OpenGraph workflows start with the same required data payload schema (`metadata`, `graph.nodes`, and `graph.edges`).

Use the following guidance to choose your path:
...
In structured graphs, traversability is defined by `relationship_kinds` in the extension definition schema, and edges inherit that setting from their kind.

Generic OpenGraph data does not define relationship traversability through an extension definition schema, so generic edges are treated as non-traversable for pathfinding and findings/analysis.

To define structured graph behavior, continue to [Extension Definition Schema](/opengraph/developer/extension-definition-schema).
