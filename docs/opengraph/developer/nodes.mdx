...
<ResponseField name="kinds" type="array" required>
  ...
  Values must match at least one [`node_kinds.name`](/opengraph/developer/extension-definition-schema#node_kinds) in the extension definition schema, which also must match at least one [`environments.environment_kind`](/opengraph/developer/extension-definition-schema#environments) (also in the extension definition schema).
</ResponseField>
