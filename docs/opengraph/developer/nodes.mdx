...
Every node must have a universally unique `id` to distinguish it from every other node in BloodHound's graph database. Use the identifier that the source system itself uses to differentiate objects — in many cases this is a GUID.
...
Node `kinds` must not overlap with existing BloodHound built-in node kinds. Use a unique prefix (for example, a namespace matching your extension) to avoid conflicts.

See [BloodHound Nodes](/resources/nodes/overview) for the full list of built-in node kinds.
