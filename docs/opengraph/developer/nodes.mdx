...
Nodes represent the entities in your graph, such as users, devices, repositories, applications, and (for findings) environments. Each node requires a stable identifier and a `kinds` array, and can also include optional properties.
...
<ResponseField name="kinds" type="array" required>
  ...
  - In structured graphs, values must match at least one [`node_kinds.name`](/opengraph/developer/graph-definition#node_kinds) in the extension definition schema, which also must match at least one [`environments.environment_kind`](/opengraph/developer/graph-definition#environments) (also in the extension definition schema) to produce <Tooltip tip="A specific subsection of an Attack Path that BloodHound Enterprise has identified as a high-value remediation point." cta="Learn more" href="/analyze-data/findings/attack-paths#findings">findings</Tooltip>.
</ResponseField>
...
<ResponseField name="properties.environmentid" type="string">
  ...
  To enable <Tooltip tip="A specific subsection of an Attack Path that BloodHound Enterprise has identified as a high-value remediation point." cta="Learn more" href="/analyze-data/findings/attack-paths#findings">findings</Tooltip> in structured graphs, define an environment node in `graph.nodes` and set `properties.environmentid` for all other node kinds to the environment node's `name`.
</ResponseField>
<ResponseField name="properties.collected" type="boolean">
  ...
  In structured graphs, only environment nodes with `properties.collected` set to `true` and a defined [`metadata.source_kind`](/opengraph/developer/metadata) are used to produce <Tooltip tip="A specific subsection of an Attack Path that BloodHound Enterprise has identified as a high-value remediation point." cta="Learn more" href="/analyze-data/findings/attack-paths#findings">findings</Tooltip>.
...
Nodes definitions must adhere to the following constraints.
