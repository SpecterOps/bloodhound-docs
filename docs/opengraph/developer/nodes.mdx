...
<ResponseField name="kinds" type="array" required>
  ...
  - Node `kinds` must not overlap with [built-in node](/resources/nodes/overview) kinds. Consider using a prefix related to your data source or environment separated by an underscore. For example, `Okta_User`.

  - In structured graphs, values must match at least one [`node_kinds.name`](/opengraph/developer/graph-definition#node_kinds) in the extension definition schema, which also must match at least one [`environments.environment_kind`](/opengraph/developer/graph-definition#environments) (also in the extension definition schema) to produce <Tooltip tip="A specific instance of an Attack Path that BloodHound Enterprise has identified as a high-value remediation point." cta="Learn more" href="/analyze-data/findings/attack-paths#findings">findings</Tooltip>.
</ResponseField>
...
- Property names should be lowercase.
  ...
  <Note>
    BloodHound accepts property names with uppercase characters, but using lowercase is a best practice. Future releases may enforce lowercase property names.
  </Note>
