...
- `property` for property-based matching
...
This is the default and most common method. It resolves an endpoint by unique internal ID.
...
<ResponseField name="start.match_by" type='"property"' required>
  Set to `property` to match against one or more of the starting node's property values.
</ResponseField>
...
<ResponseField name="end.match_by" type="string" required>
  Set to `property` to match against one or more of the ending node's property values.
</ResponseField>
