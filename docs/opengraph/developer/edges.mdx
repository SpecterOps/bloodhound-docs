...
<ResponseField name="kind" type="string" required>
  ...
  - Use a namespaced format with your extension prefix followed by an underscore and a descriptive name (for example, `Okta_ResetPassword`). This prefix must be unique and must not overlap with [built-in edge](/resources/edges/overview) kinds.
