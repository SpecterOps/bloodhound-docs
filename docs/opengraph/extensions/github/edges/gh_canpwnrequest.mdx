...
- Destination: [GH_Repository](/opengraph/extensions/github/nodes/gh_repository), [GH_Branch](/opengraph/extensions/github/nodes/gh_branch)
- Traversable: ✅
...
1. **Read access**: The role has a [GH_ReadRepoContents](/opengraph/extensions/github/edges/gh_readrepocontents) edge to the repository.
...
- **OIDC traversal requires `id-token: write`**: The attack chain from GH_CanPwnRequest through [GH_CanAssumeIdentity](/opengraph/extensions/github/edges/gh_canassumeidentity) to a cloud role is only valid if the pwn-requestable workflow or job explicitly declares `id-token: write`.
