...
<Note>
The SCIM extension is a **schema-only** extension — it does not include a collector. SCIM nodes and edges are produced by other collectors such as the [OpenHound Okta and GitHub collectors](/openhound/overview#collectors). Even in BloodHound Enterprise tenants where the official GitHub or Okta schemas are pre-installed, you must still upload the SCIM extension schema separately.
</Note>
...
2. Upload the schema to your BloodHound instance alongside the extension schemas for the collectors you are using (for example, Okta or GitHub). In BloodHound Enterprise v9.3.0 and later, verify whether the primary platform schema is already present before you upload anything else.
