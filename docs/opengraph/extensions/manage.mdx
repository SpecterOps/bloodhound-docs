...
The existing [extension developer](/opengraph/schema) documentation explains the requirements for generic graphs only. Updates for structured graphs are in progress.
</Callout>
...
| Custom node icons and colors<br/>_(schema-based)_ | <Icon icon="square-check" iconType="solid" color="#22c55e"/> | <Icon icon="square-check" iconType="solid" color="#22c55e"/> | Early access |
...
| **Basic format** | The minimal node, edge, and metadata format that all OpenGraph data payloads must conform to. Generic graphs conform to the basic format only; structured graphs conform to both the basic format _and_ an extension definition schema. |
