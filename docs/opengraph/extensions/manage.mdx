...
description: Learn how to manage OpenGraph extensions in BloodHound, including installation, updates, and deletion.
...
There's a growing ecosystem of [OpenGraph projects](/opengraph/library) that you can use to extend BloodHound's capabilities. However, _projects_ are not the same as _extensions_. An extension includes an <Tooltip tip="A file that defines graph structure, including node types, edge types, traversability behavior, and visual configurations.">extension definition schema</Tooltip>, whereas a _project_ typically includes a collector for generating data payloads only and may or may not include an extension definition schema.

Projects that do not include an extension definition schema can still produce generic graphs in BloodHound, but they do not enable enhanced features like pathfinding and relationship-based findings.

SpecterOps has built and continues to maintain the following extensions:
