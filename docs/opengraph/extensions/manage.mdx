...
Examples of supported extensions include:
...
<Note>
  Because generic graphs do not include extension-defined relationship kinds, traversability is not defined for their custom edge kinds. Generic OpenGraph edges are therefore treated as non-traversable for pathfinding and findings/analysis.
</Note>
...
<Note>
  In structured graphs, traversability is defined at the relationship kind level in the extension definition schema, and ingested edges inherit that behavior by kind.
</Note>
...
<Steps>
  ...
  <Step title="Explore and analyze">
	...
	| Feature | Description |
	|---|---|
	| Pathfinding | Use [Pathfinding](/analyze-data/explore/search#pathfinding) to identify attack paths and analyze traversable relationships across all platforms and environments, including built-in and extension-defined kinds. |
