...
The classification rules are located in the [`pz-rules/`](https://github.com/SpecterOps/GitHound/tree/main/pz-rules) directory. Each rule is a Cypher query that returns nodes to be tagged as Tier Zero. See [Privilege Zone Rules](/opengraph/extensions/githound/reference/privilege-zone-rules) for the full list of queries.
