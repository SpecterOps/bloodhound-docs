...
Each installation is linked to its parent [GH_App](/opengraph/extensions/githound/reference/nodes/gh_app) via a [GH_InstalledAs](../edgedescriptions/gh_installedas) edge. For installations with `repository_selection` set to `all`, [GH_CanAccess](../edgedescriptions/gh_canaccess) edges are created to every repository in the organization. For installations with `repository_selection` set to `selected`, repository-level edges cannot be enumerated with a PAT (requires app installation token authentication).
...
| [GH_InstalledAs](/opengraph/extensions/githound/reference/edges/gh_installedas) | [GH_App](/opengraph/extensions/githound/reference/nodes/gh_app) |

### Outbound Edges

| Edge Type | Destination Node Types |
| --------- | ---------------------- |
| [GH_CanAccess](/opengraph/extensions/githound/reference/edges/gh_canaccess) | [GH_Repository](/opengraph/extensions/githound/reference/nodes/gh_repository) |
...
| --------------------------------------------------- | --------------------------------- | ----------- | ------------------------------------------------------------------------------------------- |
| [GH_CanAccess](../edgedescriptions/gh_canaccess) | [GH_Repository](/opengraph/extensions/githound/reference/nodes/gh_repository) | -.->        | App installation can access this repository (created when `repository_selection` is `all`). |

### Inbound Edges

| Edge Name                                               | Source Node                           | Traversable | Description                                                  |
| ------------------------------------------------------- | ------------------------------------- | ----------- | ------------------------------------------------------------ |
| [GH_InstalledAs](../edgedescriptions/gh_installedas) | [GH_App](/opengraph/extensions/githound/reference/nodes/gh_app)                   | -->         | The parent app definition that this installation belongs to. |
| [GH_Contains](../edgedescriptions/gh_contains)       | [GH_Organization](/opengraph/extensions/githound/reference/nodes/gh_organization) | -.->        | The organization that contains this app installation.        |
