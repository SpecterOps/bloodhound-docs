...
Edge names must not contain hyphens (`-`). Use PascalCase and avoid special characters.

PascalCase is a naming convention where compound words are written without spaces and each word starts with an uppercase letter (e.g., `MemberOf`, `GenericAll`).
