...
Full support for BloodHound OpenGraph requires:
- BloodHound CE or Enterprise v8.0.0+
- Deployed using a PostgreSQL graph backend database (to switch to a PostgreSQL graph database, set `"graph_driver": "pg"` in your BloodHound configuration)

_Note: While much of OpenGraph will work on a Neo4j graph database, there are multiple functional and performance limitations. For full support, please migrate to a PostgreSQL graph database backend.
...
<Note>
This article is based on [Andy Robbins](https://www.linkedin.com/in/robbinsandy/)'s blog [Attack Graph Model Design Requirements and Examples](https://specterops.io/blog/2025/08/01/attack-graph-model-design-requirements-and-examples/) that goes beyond what's described here.
</Note>
