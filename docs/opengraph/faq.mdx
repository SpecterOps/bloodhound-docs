...
- Ensure each node defined in the payload has at least one relationship connecting it to another node (either predefined or otherwise defined in the payload). Including an edge that links the node to an existing object is the simplest fix.
...
If you reference an AD/AZ node when defining a relationship in an OpenGraph payload that uses this metadata field, the system inadvertently applies the `source_kind` to those non-OpenGraph nodes. When you delete the OpenGraph data, you also delete those AD/AZ nodes.
