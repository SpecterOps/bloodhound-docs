...
Every node must be linked to another node. Use one of the following strategies to link your OpenGraph nodes:

- Ensure each node has at least one relationship in the same payload. Including an edge that links the node to an existing object is the simplest fix.
...
When building hybrid paths that connect OpenGraph nodes to Active Directory (AD) or Azure (AZ) nodes, the `source_kind` metadata field is currently applied to all nodes defined and referenced in the payload. If you reference an AD/AZ node in a hybrid path, your OpenGraph `source_kind` will be applied to those non-OpenGraph nodes.
...
- **First payload (isolated subgraph)**: Upload only your OpenGraph nodes and relationships and set `source_kind` in `metadata`. Ensure that every node has at least one relationship within this payload (for example, connect nodes to each other or to a root node and add a containment edge, such as `(:OGRoot)-[:OGContains]->(:YourNode)`). Do not link to AD/AZ objects in this payload.
