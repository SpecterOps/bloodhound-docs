...
Slow ingestion speeds are usually the result of utilizing a Neo4j graph database and can be resolved by switching to a PostgreSQL backend. BloodHound OpenGraph is officially supported only on PostgreSQL backend databases.

To switch to a PostgreSQL graph database, set `"graph_driver": "pg"` in your BloodHound configuration file and restart your containers. You will need to re-ingest any data to the new database.
</Accordion>
