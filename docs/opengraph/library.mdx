...
ADAttributeHound is an [OpenGraph](/opengraph/overview) project for BloodHound that exports Active Directory custom attributes as node properties. BloodHound ingestion will make the properties merge with existing nodes otherwise it creates new nodes. ADAttributeHound enables targeted attribute collection for SharpHound Enterprise and SharpHound CE, which has the flag `-CollectAllProperties`.
