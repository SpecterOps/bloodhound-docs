...
ADAttributeHound is an [OpenGraph](/opengraph/overview) extension for BloodHound that exports Active Directory custom attributes as node properties. BloodHound ingestion will make the properties merge with existing nodes otherwise it creates new nodes. ADAttributeHound enables targeted attribute collection for SharpHound Enterprise and SharpHound CE, which has the flag `-CollectAllProperties`.
...
The collector is compatible with SpecterOps's [OpenGraph extension for GitHub](/opengraph/extensions/github/overview).
...
An [OpenHound](/opengraph/library#openhound) collector that collects resources from GitHub organizations and transforms them into usable nodes and edges for BloodHound.
...
An [OpenHound](/opengraph/library#openhound) collector that collects Jamf Pro resources and transforms them into usable nodes and edges for BloodHound.
...
The collector is compatible with SpecterOps's [OpenGraph extension for Okta](/opengraph/extensions/okta/overview).
...
An [OpenHound](/opengraph/library#openhound) collector that collects Okta resources and transforms them into usable nodes and edges for BloodHound.
...
A Python library for creating BloodHound OpenGraph JSON data that conforms to the BloodHound OpenGraph [data payload schema](/opengraph/developer/graph-data).
